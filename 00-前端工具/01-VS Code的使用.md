...
命令面板的作用是**希望解放开发者的鼠标，让一些操作和配置可以直接通过键盘进行**。如果让开发者记住所有的配置项在菜单的哪个位置是不现实的，而且有些命令并不在菜单中。有了命令面板之后，我们就可以快速进行想要的操作和配置。

如果们需要修改一些设置项，或者进行一些快捷操作，则可以通过「命令面板」来操作，效率会更高。接下来列举一些。
...
前面的快捷键那一段我们讲过，通过 「Cmd + P」可以快速搜索并打开文件。但是这种方式，只能搜到文件，搜不到文件夹。一般是用于快速打开最近编辑过的文件。

其实还有一种更巧妙的方式，可以在整个项目里，既能搜到文件，也能搜到文件夹。操作方法很简单：

> 直接在文件资源管理器输入关键字就行。搜索结果会自从出现；另外，右上角会看到一个过滤器，使用方向键进行上下移动，可以在搜索的文件和文件夹之间进行跳转。

![20211012_1616](http://img.smyhvae.com/20211012_1616.png)

点击上图中的红圈部分，则只会显示匹配的文件和文件夹。
