...
上面的代码中，如果写了两个alert()语句，则网页的效果是：弹出第一个警告框，点击确定后，继续弹出第二个警告框。
...
代码运行后，会在页面上显示一个弹窗。弹窗上有”确认“和”取消“两个按钮，点击确定返回 `true`，点击取消返回 `false`。
...
`console.log()`表示在控制台中输出。console表示“控制台”，log表示“输出”。括号里可以写字符串、数字、变量。
